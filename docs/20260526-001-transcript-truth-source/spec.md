...
본 feature는 Claude Code가 워크스페이스별로 남기는 **transcript 파일**(`<config-root>/projects/<encoded_cwd>/<session_id>.jsonl`)을 새 진실 소스로 받아들이는 정책을 정의한다. `<config-root>`는 Claude Code의 config 디렉토리로, `CLAUDE_CONFIG_DIR` 환경변수가 설정돼 있으면 그 값(예: `~/.claude-triptopaz`), 없으면 `~/.claude`다. transcript root를 `~/.claude`로 고정하면 config 홈을 옮긴 사용자의 워크스페이스 transcript를 전부 놓치므로(v0.3.11 회귀), root 결정은 반드시 `CLAUDE_CONFIG_DIR`을 우선한다. session-state 캐시(`session-state-<key>.json`)의 존재 의의는 transcript로 cover되지 않는 영역에 한해 재정의된다.
...
- **transcript root는 `CLAUDE_CONFIG_DIR`을 따른다**: Claude Code는 config 디렉토리(따라서 transcript가 쓰이는 `projects/` 루트)를 `CLAUDE_CONFIG_DIR`로 옮길 수 있다. cc-usage는 이 env가 설정돼 있으면 `<CLAUDE_CONFIG_DIR>/projects`를, 없으면 `<home>/.claude/projects`를 transcript root로 쓴다. 공백만 있는 env는 미설정으로 취급한다.
- **인코딩 방향 제약**: cwd → transcript 디렉토리 이름 변환은 forward-only deterministic 함수로 정의된다. 치환 대상 구분자는 OS에 따라 달라지므로 **`/`, `\`, `:`, `.` 을 모두 `-`로 치환**한다. POSIX는 `/`·`.`가, Windows는 `\`·`:`·`.`가 실제로 등장한다(실측: `C:\Users\zipke\GolandProjects\cc-usage` → `C--Users-zipke-GolandProjects-cc-usage`, 즉 `C:\` → `C--`). cc-usage는 cross-platform 바이너리로 배포되므로 인코딩 함수는 네 구분자를 모두 처리해 어느 OS에서든 동일 규칙으로 동작해야 한다. 디코딩(인코딩된 dirname → cwd)에 의존하는 코드 경로는 허용되지 않는다 — 인코딩이 lossy이므로 reverse는 모호하다.
