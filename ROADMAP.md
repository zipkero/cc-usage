...
5. 공식 status line 문서가 정의한 필드 의미와 cc-usage의 계산·표기가 일치한다 — 같은 payload에서 값이
   입력 형태에 따라 달라지지 않는다.
6. status line 렌더 중 네트워크 엔드포인트에 접속하지 않고 캐시 파일을 읽거나 쓰지 않으며, 텔레메트리가
   없다. stdout은 위젯 렌더 결과와 ANSI 코드만 담는다.

현재 미달인 것은 3번(§M2), 4번(§M1), 5번(§M3)이다. 1·2·6번은 v0.5.5 기준 충족한다.
...
교정한다. `20260727-001-statusline-schema-catchup`으로 SPEC·ANALYSIS가 이미 작성돼 있고, analysis의
승인 대기 3건(context 토큰 표시 기준, bool 상태 false의 표시 정책, `COLUMNS` 축소 순서)이 풀리면
IMPLEMENT로 넘어간다. `20260730-001-session-start-placeholder`가 기본 위젯 구성을 바꿨으므로 그
feature의 완료 조건 기준선과 관련 근거 문장을 먼저 갱신해야 한다.
...
- **stdin 밖 데이터 출처 도입** — OAuth usage 엔드포인트 호출로 5h/7d를 세션 첫 렌더부터 채우는 것,
  ...
  정면으로 되돌리는 일이고, 렌더 경로에 네트워크·자격증명 읽기·캐시 파일이 다시 들어오기 때문이다.
